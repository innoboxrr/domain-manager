...
export const API_ROUTE_PREFIX = 'api.domain_provider_payment.'; // Reemplaza con la ruta adecuada
	...
	return [
		{
			id: 'create',
			name: 'Create',
			callback: 'createDomainProviderPayment',
			icon: 'fa-plus',
			route: true,
			policy: false,
			params: {
...
export const showModel = (modelId, loadRelations = [], loadCounts = [], data = {}) => {
    return makeHttpRequest('get', route(API_ROUTE_PREFIX + 'show'), {
        _token: CSRF_TOKEN,
        domain_provider_payment_id: modelId,
...
export const deleteModel = (data) => {
    ...
    return makeHttpRequest('post', route(API_ROUTE_PREFIX + 'delete'), {
        _token: CSRF_TOKEN,
        _method: 'DELETE',
        domain_provider_payment_id: data.id,
