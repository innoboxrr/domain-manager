...
export const API_ROUTE_PREFIX = 'api.domain_subscription.'; // Reemplaza con la ruta adecuada
	...
	return [
		{
			id: 'create',
			name: 'Create',
			callback: 'createDomainSubscription',
			icon: 'fa-plus',
			route: true,
			policy: false,
			params: {
...
export const showModel = (modelId, loadRelations = [], loadCounts = [], data = {}) => {
    return makeHttpRequest('get', route(API_ROUTE_PREFIX + 'show'), {
        _token: CSRF_TOKEN,
        domain_subscription_id: modelId,
...
export const deleteModel = (data) => {
    ...
    return makeHttpRequest('post', route(API_ROUTE_PREFIX + 'delete'), {
        _token: CSRF_TOKEN,
        _method: 'DELETE',
        domain_subscription_id: data.id,
