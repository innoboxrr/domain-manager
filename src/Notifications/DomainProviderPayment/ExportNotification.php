...
namespace Innoboxrr\DomainManager\Notifications\DomainProviderPayment;

use Maatwebsite\Excel\Facades\Excel;
use Innoboxrr\DomainManager\Exports\DomainProviderPaymentsExports;
...
class ExportNotification extends Notification
{
    ...
    {
        return config('innoboxrrdomainmanager.notification_via', ['mail', 'database']);
    }
    ...
    {
        Excel::store(
            new DomainProviderPaymentsExports($this->data), 
            $this->path, 
            config('innoboxrrdomainmanager.export_disk', 's3')
        );
    }

    private function getSubject()
    {
        return env('APP_NAME') . ' | DomainProviderPayment export notification';
    }
